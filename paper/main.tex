...
leave-one-out input-channel ablation and extended per-seed significance tables are
deferred to the appendix (Sec.~\ref{sec:appendix}).
...
% ===== Phase 9: New-Case Generalization (NEWC-02..04) =====
\subsection{New-Case Generalization}
\label{sec:newcase}

The 16-case headline of Sec.~\ref{sec:headline} (\textbf{0.8621} macro $F_1$ for
FractureTAU vs \textbf{0.5767} for the frozen ConvLSTM reference) measures
\emph{in-distribution} performance. To probe generalization we additionally evaluate
on three \emph{freshly-ingested, held-out} microstructure cases
(\texttt{new\_MS211\_V400}, \texttt{new\_MS221\_V400}, \texttt{new\_MS231\_V400})
that were never seen during training or threshold calibration --- an
out-of-distribution (OOD) robustness check. Crucially, both models are used
\emph{frozen}: the FractureTAU seeds and the ConvLSTM reference are the same
checkpoints as the headline, and we apply the \emph{identical} AR-rollout protocol
and the single late-rollout (last-20\%) macro-$F_1$ metric --- no retraining, no
recalibration, no protocol or unit change. These cases are additive OOD evidence
and are deliberately \emph{not} merged into the 16-case headline, which stays the
distinct main result.

Table~\ref{tab:newcase} and Fig.~\ref{fig:newcase} report the head-to-head. On all
three held-out cases FractureTAU (3-seed mean $\pm$ seed std) beats the frozen
ConvLSTM, aggregating to \textbf{0.9252} $\pm$ 0.0067 vs \textbf{0.5974}
($\Delta = +0.3278$; wins 3/3). The one-sided Wilcoxon signed-rank test
(H1: median$(\mathrm{TAU}-\mathrm{CNN}) > 0$) gives $W = 6.0$, $p = 0.125$,
$n = 3$.

\begin{table}[t]
\centering
\caption{New-case generalization (held-out / OOD): late-rollout macro $F_1$ under
the identical AR protocol. FractureTAU is the 3-seed mean $\pm$ seed std; ConvLSTM
is the frozen reference. Numbers are reused verbatim from the SHA-tagged 09-08
comparison artifact (no re-derivation).}
\label{tab:newcase}
\small
\begin{tabular}{lccc c}
\toprule
Held-out case & FractureTAU & ConvLSTM & $\Delta$ & Outcome \\
\midrule
\texttt{new\_MS211\_V400} & $0.9160 \pm 0.0161$ & 0.6437 & $+0.2723$ & win \\
\texttt{new\_MS221\_V400} & $0.9277 \pm 0.0208$ & 0.6086 & $+0.3191$ & win \\
\texttt{new\_MS231\_V400} & $0.9319 \pm 0.0085$ & 0.5398 & $+0.3921$ & win \\
\midrule
\textbf{Aggregate}        & $\mathbf{0.9252 \pm 0.0067}$ & \textbf{0.5974} & $+0.3278$ & 3/3 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{newcase_generalization}
\caption{Per-case late-rollout macro $F_1$ on the three held-out (OOD) cases:
frozen FractureTAU (3-seed mean, std error bar) vs the frozen ConvLSTM reference
under the identical AR-rollout protocol. FractureTAU wins on every case.}
\label{fig:newcase}
\end{figure}

\paragraph{Honest small-sample framing ($n{=}3$).}
The evidence here is the \emph{per-case table and the 3-seed mean $\pm$ std}, not a
$p$-value. With only three paired cases the one-sided Wilcoxon's \emph{minimum}
attainable $p$ is 0.125, so it can never reach 0.05 regardless of effect size; we
therefore report $p = 0.125$ as \emph{indicative only} and make no statistical-
significance claim. The consistent per-case margins ($\Delta$ from $+0.27$ to
$+0.39$) are the substantive generalization signal.

\paragraph{Provenance.}
Every number above is reused by reference from the SHA-tagged 09-08 comparison
artifact (\texttt{new\_model/OUTPUTS/newcase\_comparison.md}), which was computed
directly from the per-frame metric CSVs and the actual checkpoint hashes. The three
frozen FractureTAU seeds are the same \texttt{best.pt} checkpoints as the headline
(\texttt{0f55814f02ae}, \texttt{6a09694abf51}, \texttt{f97f429e8d9f}); the baseline
is the frozen ConvLSTM \texttt{base\_regen} reference; and the held-out data trace to
the new-case DATASET SHA \texttt{1bd1e2282fc5} (over the six new-case cache files).
No value in this subsection is hand-typed or recomputed.
% ===== END Phase 9: New-Case Generalization =====
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{length_test_emergency_horizontal}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{f1_horizon_test_emergency_horizontal}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{stability}
...
\paragraph{Qualitative panels.}
Figure~\ref{fig:fpfn} overlays the spatial false-positive (FP) and false-negative
(FN) structure of FractureTAU's AR prediction against ground truth for the
representative failure case \texttt{test\_inclusions\_1\_2}, at three points along
the rollout. The panel exposes the dominant error mode on this case: the model
\emph{hallucinates} short crack segments on the weak, low-$G_c$ inclusions (FP,
shown in one color) before the surrounding matrix has fractured, while the
true crack front is tracked with comparatively few false negatives. The overlay is
generated directly from the committed AR-prediction artifacts (the same
no-healing binarization used for every reported number), not reconstructed.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{fpfn_inclusions_1_2}
\caption{False-positive / false-negative overlay for the representative
failure case \texttt{test\_inclusions\_1\_2} at three rollout fractions
($\sim$25/50/85\%). True positives, false positives (hallucinated cracks on weak
low-$G_c$ inclusions), and false negatives are color-coded against ground truth.
Generated from the real v1.0 AR-prediction masks (\texttt{probs.npz}/\texttt{gt.npz})
under the no-healing protocol.}
\label{fig:fpfn}
\end{figure}
...
% ===== DROP-IN ANCHOR: Phases 10–11 (SED Forecasting) — do not render yet =====
% \section{SED Forecasting}
% \label{sec:sed}
% Continuous strain-energy-density head-to-head (SEDCMP-01..06): regression
% metrics, error-accumulation curves, and pred|GT|error heatmap panels for
% FractureTAU-SED vs ConvLSTM-SED under the shared continuous-AR feedback
% (no-healing deleted for the regression target).
% ===== END DROP-IN ANCHOR: Phases 10–11 =====
...
rollout stability, and a qualitative FP/FN failure-mode panel --- shows the
improvement is physically grounded. The main remaining limitation is the
workshop-scope baseline reproduction (4 BASE / 2 SED). Future work includes
displacement-field
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.6\linewidth]{reliability}
\caption{Aggregate reliability diagram over the 16 test cases (ECE $0.0435$),
binned by predicted probability.}
\label{fig:reliability}
\end{figure}

\paragraph{Extended input-channel ablation (leave-one-out).}
Table~\ref{tab:ablation-full} gives the full leave-one-out ablation over the
optional stress channel, expanding the compact body table
(Table~\ref{tab:ablation}). We report two complementary \emph{measured}
quantities per configuration: the Phase-4 breadth-run overall macro $F_1$ (the
metric on the same footing as the headline result) and the canonical
full-continuous-rollout AR micro $F_1$ (tp/fp/fn summed over every frame at a
fixed $0.5$ threshold, before the tuned two-stage recipe and calibrated
threshold of the headline model). A dash marks a configuration that was not run
under that specific protocol: von~Mises was not given a dedicated new-model AR
rollout (it degrades accuracy and is not tied to the phase-field damage driver),
and the breadth macro $F_1$ was not logged for the pressure and SED channels.
Every entry is transcribed verbatim from a committed artifact (see
Table~\ref{tab:provenance}); none is recomputed or interpolated.

\begin{table}[hb]
\centering
\caption{Full input-channel leave-one-out ablation. Breadth macro $F_1$ is from
the Phase-4 breadth runs (\texttt{results/findings/inclusion\_false\_positives.md});
full-rollout AR micro $F_1$ is from the per-mode evaluations
(\texttt{new\_model/OUTPUTS/tau\_\{base,pressure,sed\}/RESULTS.md}). The
\texttt{extra=none} BASE configuration is the headline choice; stress-derived
channels lower the overall macro $F_1$.}
\label{tab:ablation-full}
\small
\begin{tabular}{lcc}
\toprule
Input configuration & Breadth macro $F_1$ & Full-rollout AR micro $F_1$ \\
\midrule
\texttt{extra=none} (BASE, headline) & \textbf{0.86} & 0.5644 \\
$+$ pressure                         & ---           & 0.6763 \\
$+$ von Mises stress                 & 0.75          & ---    \\
$+$ strain-energy density (SED)      & 0.58          & 0.2527 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Extended per-seed significance.}
Table~\ref{tab:perseed} reports the per-case late-rollout (last-20\%) macro
$F_1$ as mean$\pm$std over the three headline seeds (42/43/44), transcribed from
\texttt{new\_model/OUTPUTS/seed\_meanstd.csv}. The per-case mean is $0.8621$,
matching the headline of Table~\ref{tab:headline}. The one-sided Wilcoxon
signed-rank test of the per-case FractureTAU$-$ConvLSTM deltas
(\texttt{results/comparisons/tau\_headline\_vs\_base\_regen.md}) is exact with
$n{=}16$, all-nonzero: $W{=}136.0$, $p{=}\num{1.526e-5}$, median
$\Delta{=}{+}0.2285$. The spread is the honest run-to-run variation (bf16 AMP
$+$ cuDNN are not byte-deterministic); we never claim bit-determinism.

\begin{table}[hb]
\centering
\caption{Extended per-seed table: per-case late-rollout (last-20\%) macro $F_1$,
mean$\pm$std over seeds 42/43/44 (source:
\texttt{new\_model/OUTPUTS/seed\_meanstd.csv}). Overall row is the per-case mean.
Headline significance: one-sided Wilcoxon $W{=}136.0$, $p{=}\num{1.526e-5}$,
$n{=}16$.}
\label{tab:perseed}
\small
\begin{tabular}{lc}
\toprule
Case & Late-rollout macro $F_1$ (mean$\pm$std, 3 seeds) \\
\midrule
test\_MS206\_V100            & $0.9506 \pm 0.0186$ \\
test\_MS206\_V1000           & $0.9674 \pm 0.0074$ \\
test\_MS206\_V200            & $0.9483 \pm 0.0038$ \\
test\_MS206\_V400            & $0.9569 \pm 0.0102$ \\
test\_MS210\_V400            & $0.9321 \pm 0.0019$ \\
test\_MS5\_V150ms\_inc       & $0.5971 \pm 0.3230$ \\
test\_MS5\_V400ms            & $0.9305 \pm 0.0332$ \\
test\_PBX1\_V400\_true       & $0.7240 \pm 0.1292$ \\
test\_PBX\_2\_V400           & $0.7841 \pm 0.0680$ \\
test\_emergency\_horizontal  & $0.8589 \pm 0.0186$ \\
test\_horizontal\_layers\_3  & $0.9390 \pm 0.0102$ \\
test\_horizontal\_layers\_4  & $0.9359 \pm 0.0167$ \\
test\_inclusions\_1\_2       & $0.8520 \pm 0.0180$ \\
test\_inclusions\_2\_2       & $0.9344 \pm 0.0258$ \\
test\_inclusions\_3\_2       & $0.8234 \pm 0.0715$ \\
test\_inclusions\_true\_V400 & $0.6584 \pm 0.1839$ \\
\midrule
\textbf{Overall (per-case mean)} & $\mathbf{0.8621}$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Provenance and reproducibility table.}
Table~\ref{tab:provenance} is the full number$\rightarrow$source audit: every
quantitative claim in the body maps to a committed first-party artifact (cited by
its \emph{relative} repository path) and a SHA-12 prefix of that artifact's
SHA-256, plus the SHA-12 of each trained checkpoint. The file SHA-12s are computed
at build time with \texttt{sha256sum} over the committed source files. No cluster,
storage, or user path appears anywhere in this manuscript; the absolute baseline
path that appears in the comparison artifact's significance block is deliberately
\emph{not} transcribed here. The from-scratch ConvLSTM reproduction remains at the
honest \textbf{4 BASE / 2 SED} scope reported in
Sec.~\ref{sec:repro-baseline}---neither broadened nor overstated.

\noindent Source artifacts (relative repository paths; file SHA-12 prefixes):
\begin{description}
  \setlength{\itemsep}{0pt}
  \item[A1] \nolinkurl{results/comparisons/tau_headline_vs_base_regen.md}\quad(\texttt{425a4aa98601})
  \item[A2] \nolinkurl{new_model/OUTPUTS/seed_meanstd.csv}\quad(\texttt{6617a712844a})
  \item[A3] \nolinkurl{results/phys_metrics/PHYS_REPORT.md}\quad(\texttt{0c76b30cc8b8})
\end{description}

\begin{table}[hb]
\centering
\caption{Provenance / reproducibility audit. Each reported body number maps to a
committed source artifact (A1--A3 above, by relative repo path + file SHA-12) or,
for the checkpoints, to the per-seed \texttt{best.pt} SHA-12.}
\label{tab:provenance}
\small
\begin{tabular}{lll}
\toprule
Reported quantity & Value & Source / SHA-12 \\
\midrule
Headline FractureTAU macro $F_1$ & $0.8621$ & A1 \\
Headline ConvLSTM macro $F_1$    & $0.5767$ & A1 \\
Wilcoxon $p$ ($W{=}136.0$, $n{=}16$) & $\num{1.526e-5}$ & A1 \\
Per-case 3-seed mean$\pm$std (Tab.~\ref{tab:perseed}) & see Tab.~\ref{tab:perseed} & A2 \\
Macro HD95 & $39.585$ px & A3 \\
Macro boundary-$F_1$ & $0.6258$ & A3 \\
Aggregate ECE & $0.0435$ & A3 \\
Params / FLOPs / A100-h & $\num{10323201}$ / $\num{2.310e11}$ / $3.826$ & A3 \\
\midrule
FractureTAU \texttt{best.pt} (seed 42) & checkpoint & \texttt{0f55814f02ae} \\
FractureTAU \texttt{best.pt} (seed 43) & checkpoint & \texttt{6a09694abf51} \\
FractureTAU \texttt{best.pt} (seed 44) & checkpoint & \texttt{f97f429e8d9f} \\
FractureTAU \texttt{best.pt} (tuned, tau\_refined) & checkpoint & \texttt{f1a0786c519b} \\
\bottomrule
\end{tabular}
\end{table}
