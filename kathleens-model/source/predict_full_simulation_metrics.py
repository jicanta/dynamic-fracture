...
# Repo-root shim so the shared numpy-only metric seam (D-11) is importable from
# this TF-pipeline module. frac_metrics imports NEITHER framework, so pulling it
# into a TF process introduces no TF<->PyTorch coupling.
    ...
    *,
    ...
    while True:
        ...
        for i in range(B):
            ...
            # Single shared seam (D-11): replaces sklearn metrics_from_binary +
            # compute_bce_per_frame so old + new pipelines compute byte-identical
            # f1/iou/bce. Counts from the no-healed binary masks; BCE from raw probs.
            m = per_frame_metrics(gt_bin, pred_bin, pred_prob_flip)
            ...
            row = {
                # ---- canonical columns (D-13), exact names ----
